 ...
 * 
 * Best results (2023) obtained with ActivitySearch searching over all IntVars, using disjunctive constraints
 * Default (ie no search strategy) using disjunctive is also good.
 * 
 */
      ...
      while (solver.solve()) { //print the solution
         ...
         for (int job = 0; job < numJobs; job++) {
            ...
            for (int task = 0; task < numRes; task++) {
               System.out.print("(" + task + "-" + jobtask[job][task].getStart().getValue() + ") ");
            }
            System.out.println();
         }
         /* */
